...
class LightGCN(object):
    def __init__(self, data_config, pretrain_data):
        ...
        *********************************************************
        ...
        if self.alg_type in ['lightgcn']:
            self.ua_embeddings, self.ia_embeddings = self._create_lightgcn_embed()          # 初始化embedding
    ...
    def _create_lightgcn_embed(self):
        if self.node_dropout_flag:
            ...
            A_fold_hat = self._split_A_hat(self.norm_adj)                   # 分成100组
    ...
    def _convert_sp_mat_to_sp_tensor(self, X):
        ...
        return tf.SparseTensor(indices, coo.data, coo.shape)        # 这一步是转成系数的向量
